...
THREE.LoaderSupport.ConsoleLogger = (function() {
  ...
  ConsoleLogger.prototype.logWarn = function(message, additional) {
    this._createStatement(message, 'Additional content:', additional, function(
      output
    ) {
      console.warn(output)
    })
  ...
  ConsoleLogger.prototype._createStatement = function(
    message,
    addHeader,
    additional,
    logFunction
...
THREE.LoaderSupport.Callbacks = (function() {
   ...
   */
  Callbacks.prototype.setCallbackOnLoadMaterials = function(
...
THREE.LoaderSupport.Builder = (function() {
  ...
  function Builder(logger) {
    ...
    this.logger.logInfo(
      'Using THREE.LoaderSupport.Builder version: ' + LOADER_BUILDER_VERSION
    )
    this.callbacks = new THREE.LoaderSupport.Callbacks()
    this.materials = []
  ...
  Builder.prototype.buildMeshes = function(meshPayload) {
    ...
    if (Validator.isValid(callbackOnMeshAlter)) {
      ...
      if (Validator.isValid(callbackOnMeshAlterResult)) {
        if (
          !callbackOnMeshAlterResult.isDisregardMesh() &&
          callbackOnMeshAlterResult.providesAlteredMeshes()
        ) {
...
THREE.LoaderSupport.LoaderBase = (function() {
   ...
   */
  LoaderBase.prototype.checkResourceDescriptorFiles = function(
    resources,
    fileDesc
  ) {
...
THREE.LoaderSupport.WorkerRunnerRefImpl = (function() {
  ...
  WorkerRunnerRefImpl.prototype.processMessage = function(payload) {
    ...
    if (payload.cmd === 'run') {
      ...
      if (typeof parser['setLogConfig'] === 'function')
        parser.setLogConfig(logEnabled, logDebug)
...
THREE.LoaderSupport.WorkerSupport = (function() {
  ...
  var LoaderWorker = (function() {
    function LoaderWorker(logger) {
      this.logger = Validator.verifyInput(
        logger,
        new THREE.LoaderSupport.ConsoleLogger()
      )
      this._reset()
    }
    ...
    LoaderWorker.prototype.setCallbacks = function(builder, onLoad) {
      this.callbacks.builder = Validator.verifyInput(
        builder,
    ...
    LoaderWorker.prototype.run = function(payload) {
      ...
      if (!Validator.isValid(this.callbacks.builder))
      ...
      if (Validator.isValid(payload.logger)) {
        payload.logger.enabled = Validator.verifyInput(
          payload.logger.enabled,
          true
        )
        payload.logger.debug = Validator.verifyInput(
    ...
    LoaderWorker.prototype.setTerminateRequested = function(
      terminateRequested
    ) {
  ...
  var buildSingleton = function(fullName, object, internalName) {
    var objectString = ''
    var objectName = Validator.isValid(internalName)
      ? internalName
      : object.name
...
THREE.LoaderSupport.WorkerDirector = (function() {
   ...
   */
  WorkerDirector.prototype.prepareWorkers = function(
    globalCallbacks,
    maxQueueSize,
    maxWebWorkers
  ) {
  ...
  WorkerDirector.prototype._kickWorkerRun = function(prepData, supportDesc) {
    ...
    var wrapperOnMeshAlter = function(event) {
      if (Validator.isValid(globalCallbacks.onMeshAlter))
        globalCallbacks.onMeshAlter(event)
      if (Validator.isValid(prepDataCallbacks.onMeshAlter))
        prepDataCallbacks.onMeshAlter(event)
    }
  ...
  WorkerDirector.prototype._buildLoader = function(instanceNo) {
    ...
    this.workerDescription.classDef.call(
      loader,
    ...
    if (
      ...
      !Validator.isValid(loader.callbacks)
    ) {
      this.logger.logWarn(
        classDef.name +
  ...
  WorkerDirector.prototype.tearDown = function(callbackOnFinishedProcessing) {
    ...
    this.instructionQueuePointer = this.instructionQueue.length
    this.callbackOnFinishedProcessing = Validator.verifyInput(
