...
THREE.OBJLoader2 = (function() {
   ...
   */
  OBJLoader2.prototype.setMaterialPerSmoothingGroup = function(
  ...
  OBJLoader2.prototype.parseAsync = function(content, onLoad) {
    ...
    this.workerSupport = Validator.verifyInput(
      this.workerSupport,
      new THREE.LoaderSupport.WorkerSupport(this.logger)
    )
    var buildCode = function(funcBuildObject, funcBuildSingleton) {
      ...
      workerCode += funcBuildSingleton(
        'THREE.LoaderSupport.ConsoleLogger',
        THREE.LoaderSupport.ConsoleLogger
      )
      workerCode += funcBuildSingleton(
        'THREE.LoaderSupport.LoaderBase',
        THREE.LoaderSupport.LoaderBase
      )
  ...
  var Parser = (function() {
    ...
    Parser.prototype.setMaterialPerSmoothingGroup = function(
      materialPerSmoothingGroup
    ) {
    ...
    Parser.prototype.setCallbackBuilder = function(callbackBuilder) {
      if (!THREE.LoaderSupport.Validator.isValid(callbackBuilder))
        throw 'Unable to run as no "builder" callback is set.'
    ...
    Parser.prototype.processLine = function(
      ...
      var concatStringBuffer = function(
        buffer,
        bufferPointer,
        slashSpacePattern
      ) {
      ...
      switch (buffer[0]) {
        ...
        case 'l':
          ...
          this.processLinesOrPoints(
            buffer,
            bufferPointer,
            countSlashes(slashSpacePattern, slashSpacePatternPointer)
          )
          break
        ...
        case 'g':
          ...
          this.rawMesh.groupName = THREE.LoaderSupport.Validator.verifyInput(
            concatStringBuffer(buffer, bufferPointer, slashSpacePattern),
            ''
          )
          flushStringBuffer(buffer, bufferPointer)
          break
        ...
        case 'o':
          ...
          this.rawMesh.objectName = THREE.LoaderSupport.Validator.verifyInput(
            concatStringBuffer(buffer, bufferPointer, slashSpacePattern),
            ''
          )
          flushStringBuffer(buffer, bufferPointer)
        ...
        case 'mtllib':
          this.rawMesh.mtllibName = THREE.LoaderSupport.Validator.verifyInput(
            concatStringBuffer(buffer, bufferPointer, slashSpacePattern),
            ''
          )
          flushStringBuffer(buffer, bufferPointer)
        ...
        case 'usemtl':
          var mtlName = concatStringBuffer(
            buffer,
            bufferPointer,
            slashSpacePattern
          )
          if (
            ...
            THREE.LoaderSupport.Validator.isValid(mtlName)
          ) {
            this.rawMesh.activeMtlName = mtlName
    ...
    /*
      ...
      * 1: "l vertex			vertex 			..."
      */
    Parser.prototype.processLinesOrPoints = function(
      buffer,
      bufferPointer,
      slashCount
    ) {
      ...
      if (bufferLength === slashCount * 2) {
        for (length = bufferPointer; i < length; i += 2)
          this.buildFace(buffer[i], buffer[i + 1])
      } else {
    ...
    Parser.prototype.processCompletedMesh = function() {
      ...
      if (THREE.LoaderSupport.Validator.isValid(result)) {
        ...
        if (this.logger.isDebug())
          this.logger.logDebug(this.createRawMeshReport(this.inputObjectCount))
      ...
      this.callbackBuilder(
        {
          ...
          geometryType:
            this.rawMesh.faceType < 4 ? 0 : this.rawMesh.faceType === 4 ? 1 : 2,
        },
        [vertexFA.buffer],
        THREE.LoaderSupport.Validator.isValid(indexUA)
          ? [indexUA.buffer]
          : null,
        THREE.LoaderSupport.Validator.isValid(colorFA)
          ? [colorFA.buffer]
          : null,
  ...
  OBJLoader2.prototype._loadMtl = function(
    ...
    if (Validator.isValid(resource))
      this.logger.logTimeStart('Loading MTL: ' + resource.name)
    ...
    var processMaterials = function(materialCreator) {
      ...
      if (Validator.isValid(resource))
        scope.logger.logTimeEnd('Loading MTL: ' + resource.name)
      ...
      if (Validator.isValid(resource.content)) {
        processMaterials(
          Validator.isValid(resource.content)
            ? mtlLoader.parse(resource.content)
            : null
        )
